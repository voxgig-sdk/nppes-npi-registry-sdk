# nppes-npi-registry-cli

boru-driven command-line client **and** interactive REPL for the NppesNpiRegistry
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nppes-npi-registry-cli)
make build

# 2. See usage (words, entities, env vars)
./nppes-npi-registry-cli --help

# 3. Provide credentials once, via the environment
export NPPES_NPI_REGISTRY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./nppes-npi-registry-cli list search_npi

# 5. Override the API base URL for a single call
NPPES_NPI_REGISTRY_BASE=https://api.example.com ./nppes-npi-registry-cli list search_npi

# 6. No arguments -> interactive REPL
./nppes-npi-registry-cli
nppes-npi-registry> list search_npi
nppes-npi-registry> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nppes-npi-registry-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nppes-npi-registry-cli list search_npi
   ```
...
Run with no arguments to open a REPL (prompt `nppes-npi-registry>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `search_npi`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nppes-npi-registry-cli`. |
...
(`./nppes-npi-registry-cli <expr>`) and interactively (the REPL), and expressions compose the
