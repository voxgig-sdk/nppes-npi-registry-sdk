...
The NPPES API provides a real-time alternative to accessing NPPES public data, allowing users to retrieve detailed information about healthcare providers using their National Provider Identifier (NPI).

Learn more about NPPES NPI Registry at [npiregistry.cms.hhs.gov/api-page](https://npiregistry.cms.hhs.gov/api-page).

This is an unofficial SDK for the NPPES NPI Registry public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — SearchNpi — that you
...
const items = await client.SearchNpi().list()
```
...
```go
client := sdk.Test()
result, err := client.SearchNpi(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **SearchNpi** | The SearchNpi entity (list). | `/` |
...
$searchnpis = $client->SearchNpi()->list();
print_r(array_map(fn($item) => $item->data_get(), $searchnpis));
```
