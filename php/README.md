# NppesNpiRegistry PHP SDK



The PHP SDK for the NppesNpiRegistry API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->SearchNpi()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $searchnpi));
```
...
```php
$client = NppesNpiRegistrySDK::test($testopts, $sdkopts);
```
...
```
php/
├── nppesnpiregistry_sdk.php          -- Main SDK class
...
The main class (`nppesnpiregistry_sdk.php`) exports the SDK class
