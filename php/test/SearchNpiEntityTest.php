...
class SearchNpiEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = NppesNpiRegistrySDK::test($seed, null);
        $seen = iterator_to_array($base->SearchNpi(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = NppesNpiRegistryConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = NppesNpiRegistrySDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->SearchNpi(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function search_npi_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["NPPES_NPI_REGISTRY_TEST_LIVE"] === "TRUE") {
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["NPPES_NPI_REGISTRY_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
