...
//    only - a typed NppesNpiRegistryContext there would make every ctx
...
final class NppesNpiRegistryOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NppesNpiRegistryContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NppesNpiRegistrySpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NppesNpiRegistryResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NppesNpiRegistryError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NppesNpiRegistryResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NppesNpiRegistryError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NppesNpiRegistryError(is_string($code) ? $code : '', $msg);
    }
