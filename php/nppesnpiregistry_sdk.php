...
class NppesNpiRegistrySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NppesNpiRegistryHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NppesNpiRegistryHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NppesNpiRegistryFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NppesNpiRegistryFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NppesNpiRegistryUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NppesNpiRegistryHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NppesNpiRegistryHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NppesNpiRegistryHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NppesNpiRegistryError($op . "_allow",
                "NppesNpiRegistrySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NppesNpiRegistryHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NppesNpiRegistryError("graphql_error",
                "NppesNpiRegistrySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function SearchNpi($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_search_npi === null) {
                $this->_search_npi = new SearchNpiEntity($this, null);
            }
            return $this->_search_npi;
        }
        return new SearchNpiEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new NppesNpiRegistrySDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
