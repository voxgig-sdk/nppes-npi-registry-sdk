...
// NppesNpiRegistry SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NppesNpiRegistryTestFeature extends NppesNpiRegistryBaseFeature
{
    ...
    public function init(NppesNpiRegistryContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NppesNpiRegistryContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NppesNpiRegistryParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NppesNpiRegistryContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NppesNpiRegistryContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NppesNpiRegistryParam::call($ctx, $k);
