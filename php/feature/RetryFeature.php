...
// NppesNpiRegistry SDK retry feature
...
class NppesNpiRegistryRetryFeature extends NppesNpiRegistryBaseFeature
{
    ...
    public function init(NppesNpiRegistryContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NppesNpiRegistryContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NppesNpiRegistryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NppesNpiRegistryContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
