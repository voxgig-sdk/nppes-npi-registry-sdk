# NppesNpiRegistry SDK test runner
...
class NppesNpiRegistryTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NppesNpiRegistryTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NppesNpiRegistryTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NppesNpiRegistryTestRunner.getenv("NPPES_NPI_REGISTRY_TEST_LIVE")
        override = NppesNpiRegistryTestRunner.getenv("NPPES_NPI_REGISTRY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NppesNpiRegistryTestRunner.getenv(key)
        ...
        explain = NppesNpiRegistryTestRunner.getenv("NPPES_NPI_REGISTRY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NppesNpiRegistryTestRunner._test_control is not None:
            return NppesNpiRegistryTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NppesNpiRegistryTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NppesNpiRegistryTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NppesNpiRegistryTestRunner.load_test_control()
...
def load_env_local():
    NppesNpiRegistryTestRunner.load_env_local()


def env_override(m):
    return NppesNpiRegistryTestRunner.env_override(m)


def entity_data(v):
    return NppesNpiRegistryTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NppesNpiRegistryTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NppesNpiRegistryTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NppesNpiRegistryTestRunner.load_test_control()


def live_client_options():
    return NppesNpiRegistryTestRunner.live_client_options()


def live_delay_ms():
    return NppesNpiRegistryTestRunner.live_delay_ms()
