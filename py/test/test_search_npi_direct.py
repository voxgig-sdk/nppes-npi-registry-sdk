...
class TestSearchNpiDirect:

    def test_should_direct_list_search_npi(self):
        setup = _search_npi_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-search_npi", "live" if setup["live"] else "unit")
        if _skip:
...
def _search_npi_direct_setup(mockres):
    ...
    if live:
        ...
        client = NppesNpiRegistrySDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
