...
class TestSearchNpiEntity:
    ...
    def test_should_stream(self):
        ...
        base = NppesNpiRegistrySDK.test(seed, None)
        seen = list(base.SearchNpi(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = NppesNpiRegistrySDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.SearchNpi(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID JSON to run live")
        client = setup["client"]
...
def _search_npi_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("NPPES_NPI_REGISTRY_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("NPPES_NPI_REGISTRY_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("NPPES_NPI_REGISTRY_TEST_EXPLAIN") == "TRUE",
        "live": _live,
