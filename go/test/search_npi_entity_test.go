...
func TestSearchNpiEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.SearchNpi(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.SearchNpi(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		searchNpiRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.search_npi")))
...
func search_npiBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID"])
