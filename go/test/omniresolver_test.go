...
import (
	...
	omni "github.com/voxgig-sdk/nppes-npi-registry-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NppesNpiRegistryError); is {
				out := map[string]any{
					"name":    "NppesNpiRegistryError",
...
func omniCtx(arg any, client *sdk.NppesNpiRegistrySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NppesNpiRegistrySDK); is {
			client = live
