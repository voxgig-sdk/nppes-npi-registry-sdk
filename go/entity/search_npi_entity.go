...
// argument it returns the current data as an SearchNpi; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *SearchNpiEntity) DataTyped(data ...SearchNpi) SearchNpi {
	if len(data) > 0 {
		return typedFrom[SearchNpi](e.Data(asMap(data[0])))
	}
	return typedFrom[SearchNpi](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through SearchNpi (all fields
// optional at the wire level).
func (e *SearchNpiEntity) MatchTyped(match ...SearchNpi) SearchNpi {
	if len(match) > 0 {
		return typedFrom[SearchNpi](e.Match(asMap(match[0])))
	}
	return typedFrom[SearchNpi](e.Match())
}
...
func (e *SearchNpiEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
// SearchNpiListMatch and returns []SearchNpi. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *SearchNpiEntity) ListTyped(reqmatch SearchNpiListMatch, ctrl map[string]any) ([]SearchNpi, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[SearchNpi](res), nil
}
