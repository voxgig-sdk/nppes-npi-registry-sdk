...
// NppesNpiRegistry SDK feature test
//
...
class FtCtrl extends NppesNpiRegistryControl
{
...
class FtClient
{
    ...
    private NppesNpiRegistryUtility $utility;

    public function __construct(NppesNpiRegistryUtility $utility, array $options)
    {
    ...
    public function get_utility(): NppesNpiRegistryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NppesNpiRegistryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NppesNpiRegistryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NppesNpiRegistryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NppesNpiRegistryConfig::make_feature($name);
    ...
    private static function buildUrl(NppesNpiRegistrySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NppesNpiRegistryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NppesNpiRegistryResponse($fetched);
        }
    ...
    private function fail(NppesNpiRegistryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NppesNpiRegistryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
