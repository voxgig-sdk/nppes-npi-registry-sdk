...
module NppesNpiRegistryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NppesNpiRegistryResponse.new({})
    ...
    if fetch_err
      ...
      response = NppesNpiRegistryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NppesNpiRegistryResponse.new(fetched)
    else
