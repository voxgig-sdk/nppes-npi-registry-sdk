...
[MCP](https://modelcontextprotocol.io) server exposing the NppesNpiRegistry SDK as
two agent tools — `nppes-npi-registry_list` and `nppes-npi-registry_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nppes-npi-registry-mcp)
make build

# 2. Provide credentials via the environment
export NPPES_NPI_REGISTRY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nppes-npi-registry \
  -- /absolute/path/to/nppes-npi-registry-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nppes-npi-registry-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nppes-npi-registry_list: first page of records
{ "entity": "search_npi" }
{ "entity": "search_npi", "query": { } }

// nppes-npi-registry_load: one record by id
{ "entity": "search_npi", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nppes-npi-registry-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nppes-npi-registry \
     -- "$PWD"/dist/*/nppes-npi-registry-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nppes-npi-registry_list` and `nppes-npi-registry_load` tools now appear
   in new sessions. Ask the agent to *"list search_npi using nppes-npi-registry"*
   and it calls `nppes-npi-registry_list` with `{"entity":"search_npi"}`.
...
```sh
export NPPES_NPI_REGISTRY_APIKEY=sk_live_xxx            # API key
export NPPES_NPI_REGISTRY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nppes-npi-registry_list` tool
...
### Call the `nppes-npi-registry_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nppes-npi-registry_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nppes-npi-registry_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NPPES_NPI_REGISTRY_APIKEY` | API key sent with every request. |
| `NPPES_NPI_REGISTRY_BASE` | Optional override of the API base URL. |
...
```sh
./nppes-npi-registry-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nppes-npi-registry_load","arguments":{"entity":"search_npi","query":{"id":1}}}}'
```
