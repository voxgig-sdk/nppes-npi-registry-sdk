...
class SearchNpiEntity
  ...
  def make
    opts = @_entopts.dup
    SearchNpiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NppesNpiRegistryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [SearchNpi, Hash] the current SearchNpi data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NppesNpiRegistryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of SearchNpi fields)
  def match_get
  ...
  #
  # @param reqmatch [SearchNpiListMatch, Hash, nil] match filter (any subset of
  #   SearchNpi fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<SearchNpi>, Array] the matching SearchNpi items; raises NppesNpiRegistryError on failure
