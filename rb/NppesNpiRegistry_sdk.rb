...
require_relative 'NppesNpiRegistry_types'
...
class NppesNpiRegistrySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NppesNpiRegistryHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NppesNpiRegistryHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NppesNpiRegistryFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NppesNpiRegistryUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NppesNpiRegistryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NppesNpiRegistryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NppesNpiRegistryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue NppesNpiRegistryError => err
    ...
    if fetched.is_a?(Hash)
      status = NppesNpiRegistryHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NppesNpiRegistryError.new(
        "graphql_error", "NppesNpiRegistrySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NppesNpiRegistrySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
