...
class SearchNpiDirectTest < Minitest::Test
  def test_direct_list_search_npi
    setup = search_npi_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-search_npi", setup[:live] ? "live" : "unit")
    if _should_skip
...
def search_npi_direct_setup(mockres)
  ...
  if live
    ...
    client = NppesNpiRegistrySDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
