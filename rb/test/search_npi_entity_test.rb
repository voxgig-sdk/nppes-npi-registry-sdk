...
class SearchNpiEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = NppesNpiRegistrySDK.test(seed, nil)
    seen = base.SearchNpi(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.SearchNpi(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def search_npi_basic_setup(extra)
  ...
  entid_env_raw = ENV["NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID"]
