...
//     (\`new NppesNpiRegistrySDK(...)\` / \`NppesNpiRegistrySDK.test(...)\` become
//     \`NppesNpiRegistrySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NppesNpiRegistrySDK'
...
const TEST_SEED = {"entity":{"search_npi":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NppesNpiRegistrySDK(...)\` and any
// \`NppesNpiRegistrySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NppesNpiRegistrySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NppesNpiRegistrySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
