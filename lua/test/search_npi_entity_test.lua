...
describe("SearchNpiEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:SearchNpi(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:SearchNpi(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID JSON to run live")
      return
    end
...
function search_npi_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("NPPES_NPI_REGISTRY_TEST_SEARCH_NPI_ENTID")
  ...
  if env["NPPES_NPI_REGISTRY_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["NPPES_NPI_REGISTRY_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
