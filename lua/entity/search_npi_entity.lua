...
local SearchNpiEntity = {}
SearchNpiEntity.__index = SearchNpiEntity


function SearchNpiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SearchNpiEntity)
  self._name = "search_npi"
  self._client = client
...
function SearchNpiEntity:get_name()
  return self._name
end


function SearchNpiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SearchNpiEntity.new(self._client, opts)
end
...
function SearchNpiEntity:mark_deleted()
  self._deleted = true
end


function SearchNpiEntity:deleted()
  return true == self._deleted
end


function SearchNpiEntity:data_set(args)
  if args ~= nil then
...
function SearchNpiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SearchNpiEntity:match_set(args)
  if args ~= nil then
...
function SearchNpiEntity:match_get()
...
function SearchNpiEntity:stream(action, args, callopts)
...
---@param reqmatch SearchNpiListMatch
---@param ctrl? table
---@return SearchNpi[]
---@return string? err
function SearchNpiEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SearchNpiEntity:_run_op(ctx, post_done)
  local utility = self._utility
